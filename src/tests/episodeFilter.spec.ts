import { filterNameAndSummary } from "../core/episodeFilter";
...
test("test search string found in name", () => {
  const result = episodes.filter((e) => filterNameAndSummary(e, "Kingsroad"));
...
test("test search string found in summary", () => {
  const result = episodes.filter((e) => filterNameAndSummary(e, "Hound"));
